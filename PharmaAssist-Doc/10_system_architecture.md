...
| Data Layer | Supabase (PostgreSQL), Neo4j | Lưu dữ liệu quan hệ và dữ liệu graph |
...
## 8.4. Data Layer - Supabase (PostgreSQL) và Neo4j

### 8.4.1. Database quan hệ (Supabase)

Supabase (Cloud PostgreSQL) được sử dụng để lưu dữ liệu nghiệp vụ chính. Backend kết nối đến Supabase qua chuỗi kết nối (`DATABASE_URL`).
...
| Database | Supabase (Cloud PostgreSQL) | Giúp dễ triển khai và quản trị cloud database |
