...
Tài liệu này tập trung vào database quan hệ chính là Supabase (Cloud PostgreSQL). Nếu triển khai Neo4j Knowledge Graph, phần graph sẽ được mô tả chi tiết hơn trong tài liệu Graph Design riêng. Tuy nhiên, tài liệu này vẫn có thể lưu dữ liệu tham chiếu cần thiết để kết nối với graph.
...
|---|---|---|
| Database chính | Supabase (Cloud PostgreSQL) | Lưu dữ liệu nghiệp vụ quan hệ trên Cloud |
...
- Nếu backend dùng Node.js/NestJS: Supabase (Cloud PostgreSQL) + Prisma hoặc TypeORM (Khuyên dùng Prisma).
- Nếu backend dùng Spring Boot: Supabase (Cloud PostgreSQL) + JPA/Hibernate.
...
Phần này là gợi ý để nhóm có thể chạy trực tiếp trong SQL Editor của Supabase hoặc chuyển thành migration (như Prisma Migration) tùy theo ORM đang dùng.
