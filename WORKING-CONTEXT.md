...
- **2026-05-30 (Nạp Dữ Liệu Long Châu Lên Supabase):** Hoàn thành phát triển script `seed_crawled_data.ts` kết nối trực tiếp Supabase qua Prisma. Đã giải quyết triệt để 100% các xung đột duy nhất (Unique constraints) đối với các trường `brand_slug`, `category_slug`, `product_slug` và đặc biệt là `sku` (ở bảng `product_variants`). Khắc phục thành công lỗi khóa ngoại khi xóa bảng hoạt chất bằng cách đảo thứ tự xóa products lên đầu để kích hoạt cascade delete. Nạp thành công toàn bộ 15 bảng với đầy đủ 7608 sản phẩm, 15982 biến thể & giá bán, 41524 hình ảnh, 5657 thuốc, 9668 thành phần và 38805 tài liệu sản phẩm.
- **2026-05-27 (Đặc Tả Lưu Trữ Dữ Liệu Sản Phẩm):** Phân tích chi tiết các thuộc tính sản phẩm và thuốc từ tệp SQL cào dữ liệu, đối chiếu tính đồng bộ với Schema PostgreSQL hiện tại. Hoàn thành viết tài liệu đặc tả [datasave.md](file:///Users/twot/Documents/HKII_NAM_3/16_Cong_Nghe_Pham_Mem/PharmaAssist/database/datasave.md) chi tiết hóa cách thức ánh xạ dữ liệu, tối ưu hóa các trường khóa ngoại sang ID quan hệ (`bigint`), và giải thích lý do loại bỏ các trường metadata cào dữ liệu dư thừa.
