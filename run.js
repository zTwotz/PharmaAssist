...
async function doSetup() {
  ...
  try {
    // 0. Kiểm tra và copy .env
    console.log(`${COLORS.bright}[0/3] Đang kiểm tra cấu hình biến môi trường...${COLORS.reset}`);
    const backendEnvPath = path.join(__dirname, 'backend', '.env');
    const backendEnvExamplePath = path.join(__dirname, 'backend', '.env.example');
    if (!fs.existsSync(backendEnvPath) && fs.existsSync(backendEnvExamplePath)) {
      fs.copyFileSync(backendEnvExamplePath, backendEnvPath);
      console.log(`${COLORS.green}✔ Đã tự động tạo file backend/.env từ .env.example.${COLORS.reset}`);
    }
    ...
    if (!fs.existsSync(frontendEnvPath) && fs.existsSync(frontendEnvExamplePath)) {
      ...
      console.log(`${COLORS.green}✔ Đã tự động tạo file frontend/.env.local từ .env.example.${COLORS.reset}`);
    }
    console.log();
    // A. Cài đặt Backend
...
// Chức năng hiển thị hướng dẫn
function showDocs() {
  console.log(`\n${COLORS.magenta}${COLORS.bright}========== HƯỚNG DẪN CÀI ĐẶT & TRIỂN KHAI ==========${COLORS.reset}\n`);
  console.log(`${COLORS.cyan}${COLORS.bright}🚀 Môi trường Phát triển (Local Development) - Khuyến nghị${COLORS.reset}`);
  console.log(`1. Chạy lệnh: ${COLORS.yellow}node run.js${COLORS.reset} và chọn ${COLORS.yellow}[1] Cài đặt dự án (Setup)${COLORS.reset}.`);
  console.log(`2. Mở file ${COLORS.yellow}backend/.env${COLORS.reset} và cập nhật chuỗi kết nối DATABASE_URL.`);
  console.log(`3. Chạy lệnh: ${COLORS.yellow}npx prisma db push${COLORS.reset} trong thư mục backend để tạo cấu trúc bảng.`);
  console.log(`4. Chạy lại lệnh: ${COLORS.yellow}node run.js${COLORS.reset} và chọn ${COLORS.yellow}[2] Chạy đồng thời dự án (Start)${COLORS.reset}.\n`);

  console.log(`${COLORS.cyan}${COLORS.bright}🌍 Môi trường Sản xuất (Production Deployment)${COLORS.reset}`);
  console.log(`${COLORS.bright}Backend (NestJS):${COLORS.reset}`);
  console.log(`- Cài đặt thư viện: ${COLORS.yellow}npm install --omit=dev${COLORS.reset}`);
  console.log(`- Build dự án: ${COLORS.yellow}npx prisma generate && npm run build${COLORS.reset}`);
  console.log(`- Chạy server: ${COLORS.yellow}npm run start:prod${COLORS.reset}`);
  
  console.log(`\n${COLORS.bright}Frontend (Next.js):${COLORS.reset}`);
  console.log(`- Khuyến nghị deploy trên Vercel để tự động tối ưu hóa.`);
  console.log(`- Nếu tự host trên Node.js (như VPS): ${COLORS.yellow}npm run build && npm run start${COLORS.reset}`);
  
  console.log(`\nĐể xem toàn bộ thông tin chi tiết, vui lòng mở tệp ${COLORS.yellow}README.md${COLORS.reset}.\n`);
}
...
function showInteractiveMenu() {
  ...
  console.log(`${COLORS.cyan}  [0] Xem Hướng dẫn Cài đặt & Triển khai${COLORS.reset}`);
  console.log(`${COLORS.cyan}  [1] Cài đặt dự án (Setup)       - Tải thư viện, sinh Prisma Client, copy .env${COLORS.reset}`);
  ...
  rl.question(`${COLORS.bright}Lựa chọn của bạn (0-5): ${COLORS.reset}`, async (answer) => {
...
function main() {
  ...
  switch (action) {
    ...
    case '-h':
      ...
      console.log(`  node run.js docs       - Xem hướng dẫn cài đặt và triển khai dự án`);
      console.log(`  node run.js setup      - Cài đặt dependency, cấu hình thư mục, copy .env tự động`);
