...
BEGIN
    ...
    VALUES ('IMP-' || floor(random() * 1000000)::int || extract(epoch from now())::int, v_supplier_id, v_warehouse_id, NOW(), 0, 'COMPLETED', NOW(), 'System Seed', 'Initial stock import for testing')
    RETURNING id INTO v_stock_import_id;

    -- 5. Seed 50 random medicines
    FOR v_medicine IN 
    ...
    LOOP
        ...
        v_batch_num := 'BATCH-' || floor(random() * 10000)::int || '-' || v_medicine.medicine_id || '-' || v_medicine.variant_id;
